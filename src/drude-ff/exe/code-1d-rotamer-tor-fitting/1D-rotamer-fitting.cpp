...
//	Read_Parameters("saved-para.dat");
	...
	FailCount = 0;
	int check = nlopt_optimize(opt, Para_List, &Chi_SQ);
	printf("check %d\n",check);
...
char szName_Old_Rtf[]="mol-tor.rtf" ;	//"mol.rtf";
char szName_New_Rtf[]="mol-tor-new.rtf";	//"mol-esp.rtf";
...
double CalObjectiveFunction(double Para_List[], int Get_E_Rotamer)
{
	...
	MPI_Allreduce(&Chi_SQ_Rotamer_Local, &Chi_SQ_Rotamer, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
int main(int argc, char **argv)
{
	...
	Mol_ESP.ReadPSF(szName_XPSF, 0, 1);	// to delete the last atom, test charge
	Read_Rtf_File();
