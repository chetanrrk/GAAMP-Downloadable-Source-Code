...
do
  if [ -e ./fitting-1d-$NumTorsion.dat ]; then
    ...
    echo "plot \"fitting-1d-$NumTorsion.dat\" using 1:2 t \"QM\" with linespoints pt 6 lw 2 ps 2 lc rgb \"black\", \"fitting-1d-$NumTorsion.dat\" using 1:3 t \"MM\" with linespoints pt 1 lw 2 ps 2 lc 1" >> plot.sh
