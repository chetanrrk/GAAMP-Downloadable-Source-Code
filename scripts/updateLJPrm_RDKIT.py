...
def detectAtomsInCycle(pdb):
    ...
    """
    cycle = []	  
    m = Chem.MolFromPDBFile(pdb) #Chem.MolFromMol2File(mol2)
...
def mapDef1ToDef2(atomsDef1,allAtomTypesDef1,allAtomTypesDef2):
	...
	mapD1_D2[caps(k)] = [caps(mapAtom_Types_Def1[k]),caps(allAtomTypesDef2[k])]
    return mapD1_D2	
    ...
    for line in fin:
        ...
        if len(token)>0:
           ...
           if bondFlag==1 and token[0][:4]!="BOND" :
              bondRecord[caps(token[0])+"-"+caps(token[1])] = [token[2],token[3]]
           if angleFlag==1 and token[0][:5]!="ANGLE":
              angleRecord[caps(token[0])+"-"+caps(token[1])+"-"+caps(token[2])] = [token[3],token[4]]
           if dihedFlag==1 and token[0][:5]!="DIHED":
              a1 = caps(token[0]); a2 = caps(token[1]); a3 = caps(token[2]); a4 = caps(token[3])
...
def updateImpropPrm(orgImprop,mapD1_D2,impropPrm):
    ...
    for improp in orgImprop:
        ...
        elif impropPrm.has_key("X-"+oldAtomType2+"-"+oldAtomType3+"-"+oldAtomType4):
                newImpropPrm = impropPrm["X-"+oldAtomType2+"-"+oldAtomType3+"-"+oldAtomType4]
	elif impropPrm.has_key(oldAtomType1+"-"+oldAtomType2+"-"+oldAtomType3+"-X"):
		newImpropPrm = impropPrm[oldAtomType1+"-"+oldAtomType2+"-"+oldAtomType3+"-X"]
...
def run(pdb,ac_rtf,ac_prm,useOptLJ,isAdditive):

    cycle = detectAtomsInCycle(pdb) 
...
if __name__=="__main__":
    ...
    useOptLJ = True
    isAdditive = True
    run(pdb,ac_rtf,ac_prm,useOptLJ,isAdditive) 
